# 👩‍💻 Rafaela Cristina | QA Automation

Hi there 👋  

Sou profissional da área da saúde em transição estratégica para Tecnologia, com foco em **Qualidade de Software e Testes Automatizados**.  
Trago para o QA uma base forte em protocolos, segurança, análise crítica e controle de qualidade.

---

## 🎓 Formação & Especializações

🎓 Análise e Desenvolvimento de Sistemas  
🎓 Pós-graduanda em QA e Testes de Software  
🎓 Bootcamp Quality Assurance – Atlântico Avanti  
🎓 Bootcamp Java – Deloitte  
🎓 Desenvolvimento Frontend – Programaria & Ada Tech  
🎓 Curso de Desenvolvimento Fullstack (JavaScript, React e Node.js)

---

## 🎯 Foco Profissional

✔️ QA Automation  
✔️ Testes Funcionais e Não Funcionais  
✔️ Testes de Regressão  
✔️ Criação de Casos de Teste  
✔️ Evidências e Relatórios de Bugs  
✔️ Page Object Model (POM)  
✔️ Qualidade em aplicações Web  

---

## 🧪 Stack de Automação

### ☕ Java Ecosystem
- Selenium WebDriver  
- JUnit  
- TestNG  
- Maven  

### 🟨 JavaScript Ecosystem
- Cypress  
- Node.js  

---

## 🛠️ Tecnologias & Ferramentas

<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/selenium/selenium-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cypressio/cypressio-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" width="45" height="45"/>
</p>


---

## 🔍 O que você vai encontrar nos meus repositórios

- 📂 Projetos de automação com Selenium + Java  
- 📂 Projetos de automação com Cypress  
- 📋 Modelos de casos de teste  
- 🐞 Simulações de relatórios de bugs  
- 🔄 Estrutura com Page Object Model  

---

## 📈 Em Evolução

Minha missão é garantir que aplicações sejam seguras, estáveis e confiáveis.  
Qualidade não é detalhe — é requisito.

---

## 📫 Contato

- 💼 LinkedIn: [(https://www.linkedin.com/in/rafaela-souzadev/)]
- 📧 Email: [(rafacris87@gmail.com)]